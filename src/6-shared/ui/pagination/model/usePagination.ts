...
export const usePagination = ({
  ...
  return useMemo(() => {
    ...
    const isShowRightDots =
      rightSiblingIndex <= totalPageCount - RIGHT_ITEMS_COUNT + 1;
    ...
    if (isShowLeftDots && !isShowRightDots) {
      const rightRange = range(
        totalPageCount - RIGHT_ITEMS_COUNT + 1,
        totalPageCount,
      );
    ...
    if (isShowLeftDots && isShowRightDots) {
      const middleRange = range(
        leftSiblingIndex - SIBLING_ITEM_COUNT,
        rightSiblingIndex + SIBLING_ITEM_COUNT,
      );
